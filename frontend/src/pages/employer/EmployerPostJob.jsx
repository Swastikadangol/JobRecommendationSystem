...
import {
  ...
  Clock, Wifi, Users, Tag, CheckCircle,
  Lightbulb, Eye, Calendar, DollarSign
} from 'lucide-react'
...
const INITIAL = {
  jobTitle: '', jobDescription: '', jobType: '', workMode: '',
  requiredSkills: '', location: '', salaryRange: '', deadline: '',
  minimumEducationLevel: '', minYearsExperience: '',
  responsibilities: '', benefits: '', customSalary: false,
}

const TIPS = [
  { icon: '✏️', text: 'Clear job titles get 3x more applications than vague ones.' },
  { icon: '📋', text: 'Include day-to-day responsibilities so candidates know what to expect.' },
  { icon: '💰', text: 'Jobs with salary ranges get significantly more qualified applicants.' },
  { icon: '🎯', text: 'List only the skills truly required — not a wishlist.' },
  { icon: '⏳', text: 'Set a realistic deadline — at least 2 weeks from today.' },
]

function OptionCard({ option, selected, onClick }) {
  return (
    <button type="button" onClick={onClick}
      className={`w-full text-left px-3 py-2.5 rounded-xl border-2 transition-all duration-150 ${selected
...
export default function EmployerPostJob() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await employerApi.postJob(user.profileId, {
      ...
      addToast('Job posted! It will go live once approved by admin.', 'success')
    ...
    <div className="animate-fadeIn">
      ...
      <div className="mb-5">
        <div className="flex items-center gap-2 mb-1">
          <CirclePlus className="w-5 h-5 text-brand-600" />
          <h1 className="font-display text-2xl font-semibold text-slate-900 dark:text-slate-100">Post a Job</h1>
        </div>
          ...
          <Section icon={Briefcase} title="Basic Information" subtitle="Job title and description">
            ...
            <Field label="Job Description" required error={errors.jobDescription}
              ...
              <textarea rows={5} className="input resize-none"
                placeholder="We are looking for a talented developer to join our growing team…"
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {SALARY_RANGES.map(r => (
                    <button key={r} type="button" onClick={() => { set('salaryRange', r); set('customSalary', false) }}
                      className={`text-xs px-2 py-2 rounded-lg border transition-all text-center ${form.salaryRange === r && !form.customSalary
                  ...
                  <button type="button" onClick={() => { set('customSalary', true); set('salaryRange', '') }}
                    className={`text-xs px-2 py-2 rounded-lg border transition-all ${form.customSalary
                        ? 'border-brand-500 bg-brand-50 dark:bg-brand-500/10 text-brand-700 dark:text-brand-300 font-medium'
        ...
        <div className="w-full lg:w-72 flex-shrink-0 space-y-4 lg:sticky lg:top-6">
          ...
          <div className="card">
            ...
            <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full mb-3 overflow-hidden">
              <div className={`h-full rounded-full transition-all duration-500 ${pct === 100 ? 'bg-emerald-500' : 'bg-brand-500'}`}
                style={{ width: `${pct}%` }} />
            </div>
          ...
          {/* Process steps */}
          <div className="card">
            <p className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-3">What happens next?</p>
            <div className="space-y-2.5">
              {[
                { n: '1', text: 'Your job is submitted for admin review' },
                { n: '2', text: 'Admin approves and it goes live' },
                { n: '3', text: 'Candidates start applying' },
                { n: '4', text: 'You review applicants & update status' },
              ].map(s => (
                <div key={s.n} className="flex items-start gap-2.5">
                  <div className="w-5 h-5 rounded-full bg-brand-100 dark:bg-brand-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                    <span className="text-xs font-bold text-brand-600 dark:text-brand-400">{s.n}</span>
                  </div>
